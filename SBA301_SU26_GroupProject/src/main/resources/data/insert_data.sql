...
-- ChapterStatus enum: FREE, LOCKED, UNLOCKED
-- First 5 chapters per novel are FREE status with coin_price = 0.
-- Remaining chapters are LOCKED status with coin_price > 0.
...
-- First 5 chapters: status = 'FREE' and coin_price = 0.
-- Remaining chapters: status = 'LOCKED' and coin_price > 0.
-- -------------------------------------------------------------------------
...
DECLARE @ChapterStatus NVARCHAR(10);
...
BEGIN
    ...
    BEGIN
        SET @ChapterId = NEWID();
        SET @ChapterStatus = CASE WHEN @ChapterNo <= 5 THEN 'FREE' ELSE 'LOCKED' END;
        ...
        INSERT INTO chapters (id, novel_id, chapter_number, title, slug, content, status, coin_price, view_count, created_at, updated_at)
        VALUES (
            @ChapterId,
            @CurrentNovelId,
            @ChapterNo,
...
VALUES
(@PaymentId, @UserId, 100000, 1000, 'SUCCESS', 'vnpay', 'VNP123456789', DATEADD(DAY, -4, GETDATE()));

INSERT INTO coin_transactions (id, user_id, type, amount, balance_after, ref_id, note, created_at)
VALUES
(NEWID(), @UserId, 'TOPUP', 1000, 1000, @PaymentId, N'Coin deposit via VNPAY', DATEADD(DAY, -4, GETDATE())),
(NEWID(), @UserId, 'UNLOCKED_CHAPTER', 25, 975, @FirstPaidChapterId, N'Unlocked chapter 6 of Shadow Academy', DATEADD(DAY, -1, GETDATE()));
...
UNION ALL
SELECT N'Locked Chapters count', COUNT(*) FROM chapters WHERE status = 'LOCKED'
UNION ALL
