...
public class ChapterServiceImpl implements ChapterService {
    ...
    public ChapterResponseDTO createChapter(UUID novelId, ChapterRequestDTO requestDTO, String authorEmail) {
        ...
        Novel novel = novelRepository.findById(novelId)
                .orElseThrow(() -> new RuntimeException("Không tìm thấy truyện tương ứng."));
        // 2. Xác thực quyền sở hữu (Tác giả)
        if (!novel.getAuthor().getEmail().equals(authorEmail)) {
            throw new RuntimeException("Bạn không có quyền đăng chương cho bộ truyện này.");
        }
        // 3. Nghiệp vụ kiểm tra coin đối với chương trả phí
        if (!requestDTO.status().equals(ChapterStatus.FREE) && (requestDTO.coinPrice() == null || requestDTO.coinPrice() <= 0)) {
            throw new RuntimeException("Chương trả phí bắt buộc phải có giá coin lớn hơn 0.");
        }
        // 4. Tự động tính số thứ tự chương tiếp theo (Ngăn ngừa lỗi tác giả nhập trùng số chương)
        Integer maxChapterNum = chapterRepository.findMaxChapterNumberByNovelId(novelId);
        Integer nextChapterNum = maxChapterNum + 1;
        // 5. Tạo mới thực thể chương truyện
        ...
        // 6. Cập nhật thời gian update mới nhất của Truyện (để đẩy truyện lên danh sách vừa cập nhật)
    ...
    public ChapterResponseDTO getChapterDetails(UUID novelId, Integer chapterNumber, String userEmail) {
        Chapter chapter = chapterRepository.findByNovelIdAndChapterNumber(novelId, chapterNumber)
                .orElseThrow(() -> new RuntimeException("Không tìm thấy chương truyện yêu cầu."));
    ...
    public ChapterResponseDTO updateChapter(UUID chapterId, ChapterRequestDTO requestDTO, String authorEmail) {
        Chapter chapter = chapterRepository.findById(chapterId)
                .orElseThrow(() -> new RuntimeException("Không tìm thấy chương truyện cần sửa."));
        if (!chapter.getNovel().getAuthor().getEmail().equals(authorEmail)) {
            throw new RuntimeException("Bạn không có quyền chỉnh sửa chương này.");
        }
        if (!requestDTO.status().equals(ChapterStatus.FREE) && (requestDTO.coinPrice() == null || requestDTO.coinPrice() <= 0)) {
            throw new RuntimeException("Chương trả phí bắt buộc phải có giá coin lớn hơn 0.");
        }
    ...
    public void deleteChapter(UUID chapterId, String authorEmail) {
        Chapter chapter = chapterRepository.findById(chapterId)
                .orElseThrow(() -> new RuntimeException("Không tìm thấy chương truyện cần xóa."));
        if (!chapter.getNovel().getAuthor().getEmail().equals(authorEmail)) {
